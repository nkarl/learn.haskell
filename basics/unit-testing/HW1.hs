...
getMonths iL n f = helper iL f n []
  where
    -- the First Helper: to simplify the functionality
    helper [] _ _ buf = reverse buf             -- the absolute base case, when the list is finally empty
                                                -- the recursive case
    helper (monthList : log) f n buf = helper log f n (helper2 monthList f n buf) 
      where
        -- the Second Helper: to check if element exists and and whether its value (the number of cans)
        -- satisfies the requirement
        helper2 (m, []) f n buf = buf           -- the absolute base case
        -- helper2 (m, [(x, y)]) f n buf        -- the base case where there is only one element left in the list
        --   | (x == f) && (y > n) = m : buf -- if element is found, and if cans > than logged value, it's valid and should be added to buf
        --   | otherwise = buf -- otherwise, just returns the buffer
